...
namespace UnityEditor
{
	internal class ExportRawHeightmap : TerrainWizard
	{
		...
		public ExportRawHeightmap.ByteOrder m_ByteOrder = ExportRawHeightmap.ByteOrder.Mac;
		...
		private void WriteRaw(string path)
		{
			...
			if (this.m_Depth == ExportRawHeightmap.Depth.Bit16)
			{
				...
				for (int i = 0; i < heightmapHeight; i++)
				{
					for (int j = 0; j < heightmapWidth; j++)
					{
						...
						int value = Mathf.RoundToInt(heights[i, j] * num);
			...
			else
			{
				...
				for (int k = 0; k < heightmapHeight; k++)
				{
					...
					{
						int num4 = l + k * heightmapWidth;
						int value3 = Mathf.RoundToInt(heights[k, l] * num3);
