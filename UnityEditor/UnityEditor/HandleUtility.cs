...
namespace UnityEditor
{
	public sealed class HandleUtility
	{
		...
		public static float CalcLineTranslation(Vector2 src, Vector2 dest, Vector3 srcPosition, Vector3 constraintDir)
		{
			...
			Vector2 vector2 = current.WorldToScreenPoint(srcPosition);
			Vector2 vector3 = current.WorldToScreenPoint(srcPosition + constraintDir * num);
			Vector2 x = dest;
			Vector2 x2 = src;
			if (vector2 == vector3)
			{
		...
		public static Vector2 WorldToGUIPoint(Vector3 world)
		{
			...
			if (current)
			{
				Vector2 absolutePos = current.WorldToScreenPoint(world);
