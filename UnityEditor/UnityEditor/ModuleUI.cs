...
namespace UnityEditor
{
	internal abstract class ModuleUI : SerializedModule
	{
		...
		protected static Rect GetControlRect(int height)
		{
			...
			gUIStyle.margin = new RectOffset(0, 0, 2, 2);
			return GUILayoutUtility.GetRect(0f, (float)height, gUIStyle);
		}
		...
		{
			ModuleUI.GUIToggleWithFloatField(EditorGUIUtility.TempContent(name), boolProp, floatProp);
		}
		...
		{
			ModuleUI.GUIToogleWithIntField(EditorGUIUtility.TempContent(name), boolProp, floatProp, invertToggle);
		}
