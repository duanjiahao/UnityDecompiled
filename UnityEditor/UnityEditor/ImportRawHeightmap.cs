...
namespace UnityEditor
{
	internal class ImportRawHeightmap : TerrainWizard
	{
		...
		public ImportRawHeightmap.ByteOrder m_ByteOrder = ImportRawHeightmap.ByteOrder.Mac;
		...
		private void ReadRaw(string path)
		{
			...
			else
			{
				...
				for (int k = 0; k < heightmapHeight; k++)
				{
					...
					{
						int num6 = Mathf.Clamp(l, 0, this.m_Width - 1) + Mathf.Clamp(k, 0, this.m_Height - 1) * this.m_Width;
						byte b2 = array[num6];
						float num7 = (float)b2 * num5;
						array2[k, l] = num7;
					}
				}
			}
