		...
		public static void DoGUI(string title, ref bool show, ref Vector2 scrollPos, LayerMatrixGUI.GetValueFunc getValue, LayerMatrixGUI.SetValueFunc setValue)
		{
			...
			if (show)
			{
				scrollPos = GUILayout.BeginScrollView(scrollPos, new GUILayoutOption[]
				{
					...
					GUILayout.MaxHeight((float)(100 + (num + 1) * 15))
				});
				Rect rect = GUILayoutUtility.GetRect((float)(15 * num + 100), 100f);
				...
				for (int j = 0; j < 32; j++)
				{
					...
					{
						float num3 = (float)(130 + (num - num2) * 15) - (topmostRect.width + scrollPos.x);
						if (num3 < 0f)
						{
							num3 = 0f;
						}
						Vector3 pos = new Vector3((float)(130 + 15 * (num - num2)) + vector.y + vector.x + scrollPos.y - num3, vector.y + scrollPos.y, 0f);
						...
						GUI.Label(new Rect(2f - vector.x - scrollPos.y, scrollPos.y - num3, 100f, 15f), LayerMask.LayerToName(j), "RightLabel");
						num2++;
					}
				}
				...
				for (int k = 0; k < 32; k++)
				{
					...
					{
						int num4 = 0;
						Rect rect2 = GUILayoutUtility.GetRect((float)(30 + 15 * num + 100), 15f);
						GUI.Label(new Rect(rect2.x + 30f, rect2.y, 100f, 15f), LayerMask.LayerToName(k), "RightLabel");
						for (int l = 31; l >= 0; l--)
						{
							if (LayerMask.LayerToName(l) != string.Empty)
							{
								if (num4 < num - num2)
								{
									...
									bool flag2 = GUI.Toggle(new Rect(130f + rect2.x + (float)(num4 * 15), rect2.y, 15f, 15f), flag, content);
									if (flag2 != flag)
									{
										setValue(k, l, flag2);
									}
