	...
	internal class PopupList : PopupWindowContent
	{
		...
		private void DrawCustomTextField(EditorWindow editorWindow, Rect windowRect)
		{
			...
			if ((GUI.Button(position, GUIContent.none, (!(text2 != string.Empty)) ? PopupList.s_Styles.customTextFieldCancelButtonEmpty : PopupList.s_Styles.customTextFieldCancelButton) && text2 != string.Empty) || flag4)
			{
				...
				PopupList.s_RecycledEditor.content.text = empty;
				text2 = (EditorGUI.s_OriginalText = empty);
				PopupList.s_RecycledEditor.pos = 0;
				PopupList.s_RecycledEditor.selectPos = 0;
				flag = false;
			}
			if (text != text2)
			{
				this.m_EnteredText = ((0 > PopupList.s_RecycledEditor.pos || PopupList.s_RecycledEditor.pos >= text2.Length) ? text2 : text2.Substring(0, PopupList.s_RecycledEditor.pos));
				if (flag)
				{
					this.UpdateCompletion();
				}
		...
		private void UpdateCompletion()
		{
			IEnumerable<string> source = 
				from element in this.m_Data.GetFilteredList(this.m_EnteredText)
				select element.text;
			...
			else
			{
				...
				{
					if (this.m_SelectedCompletionIndex >= source.Count<string>())
					{
		...
		private void AdjustRecycledEditorSelectionToCompletion()
		{
			...
			{
				PopupList.s_RecycledEditor.content.text = this.m_EnteredTextCompletion;
				EditorGUI.s_OriginalText = this.m_EnteredTextCompletion;
				PopupList.s_RecycledEditor.pos = this.m_EnteredText.Length;
				PopupList.s_RecycledEditor.selectPos = this.m_EnteredTextCompletion.Length;
			}
		}
